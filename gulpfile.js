 ...
 * otherwise it's dev.
 */
const buildEnv = process.env.NODE_ENV ? 'production' : 'development';
...
function jsBundle(callback) {
  const host = config.get('webpackDevServer.host');
  const port = config.get('webpackDevServer.port');
  ...
  var webpackConfig = {
    ...
    resolve: {
      extensions: [ '', '.jsx', '.js' ],
      modulesDirectories: [ 'shared/node_modules', 'node_modules' ]
    },
    target: 'web',
    module: {
  ...
  if (!isWatching) {
    ...
    server.listen(config.get('webpackDevServer.port'));
  }
}
...
function browserSyncInitialize() {
  ...
  browserSync.init({
    ...
    ui: false,
    logLevel: 'silent',
    port: config.get('browserSyncServer.port')
  });
}
